...
- [Money Tree approval-revoke-mcp](https://github.com/sebastiancoombs/approval-revoke-mcp) — live x402 MCP for EVM token approvals: `/v1/wallet/approvals` ($0.10), `/v1/wallet/approvals_risk` ($0.20 composite 0–100 risk score), `/v1/spender/reputation` ($0.05). Multichain (eth/base/arb/op/polygon). USDC on Base. No keys.
- [Money Tree tx-bundle-cost-mcp](https://github.com/sebastiancoombs/tx-bundle-cost-mcp) — live x402 MCP for multi-step EVM tx cost estimation: `/v1/bundle/estimate_cost` ($0.20 composite swap+bridge+transfer with gas+slippage+bridge fees, expected + p95), `/v1/step/estimate_cost` ($0.05). Chainlink native/USD feeds for USD pricing. No keys.
- [Money Tree wallet-cex-flows-mcp](https://github.com/sebastiancoombs/wallet-cex-flows-mcp) — live x402 MCP for wallet ↔ CEX flow analysis: `/v1/wallet/cex_flows` ($0.30 composite per-CEX deposit/withdrawal counts + USDC/USDT/ETH atomic totals across 5 chains), `/v1/wallet/cex_attribution` ($0.05), `/v1/cex/recent_inflows` ($0.10). Curated 8-exchange registry. No keys.
- [Money Tree token-distribution-mcp](https://github.com/sebastiancoombs/token-distribution-mcp) — live x402 MCP for ERC-20 token distribution analysis: `/v1/token/distribution` ($0.30 composite top-10 holders + Gini concentration + LP depth + recent unlocks + GoPlus security + smart-money holder count), `/v1/token/top_holders` ($0.05), `/v1/token/unlock_calendar` ($0.07). Multichain. No keys.
- [Money Tree wallet-funding-trace-mcp](https://github.com/sebastiancoombs/wallet-funding-trace-mcp) — live x402 MCP for wallet funding/origin tracing: `/v1/wallet/funding_trace` ($0.30 multi-hop predecessor walk + clean/tc_tainted/sanctioned/mixer_tainted/multi_hop_cex/fresh_mint classification), `/v1/wallet/funding_summary` ($0.10), `/v1/wallet/funding_path` ($0.05). 4 bundled registries (CEX + mixers + sanctioned + bridges). No keys.

...
- [krewe](https://www.krewe.world) - Decentralized AI inference network on Base. Every `/v2/predict` call fans out to 3 independent miner nodes; 2-of-3 byte-equal consensus required before result settles. 4 pay-per-call kinds: `text.structure` ($0.005), `text.embed` ($0.01), `web.scrape` ($0.02), `text.complete` ($0.05, quantized SLM via Qwen2.5-0.5B). Settlement via EIP-3009 `transferWithAuthorization` on Base USDC. Open source orchestrator + miner + [TypeScript SDK](https://github.com/krewe-AI/krewe/tree/main/clients/x402-client) — [github](https://github.com/krewe-AI/krewe).

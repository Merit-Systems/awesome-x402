...
- [MAKO](https://mako.pollinateresearch.com) - Neutral trust + signal layer for x402 agent commerce on Base. Eight paid endpoints (route, verify, pulse, pricing, reputation, markets aggregator, governance proposal signal + weekly brief). Every response is EIP-191 signed over RFC 8785 canonical JSON. Settled in USDC via the Coinbase CDP facilitator. Public Pulse scoreboard at [`/pulse`](https://mako.pollinateresearch.com/pulse).
...
- [Onyx Bazaar](https://onyx-actions.onrender.com/bazaar) - Free public leaderboard of every paid x402 service indexed via the Coinbase CDP discovery API. Refreshed every 15 min, four views (top by volume / unique payers / recently active / cheapest), JSON
- variant at `/bazaar.json`. Complementary CDP-only slice next to x402Scan's multi-source view.
...
- [@pollinateresearch/mako-mcp (npm)](https://www.npmjs.com/package/@pollinateresearch/mako-mcp) - MCP server exposing MAKO's 8 paid x402 endpoints (route, verify, pulse, pricing, reputation, markets aggregator, governance proposal signal + weekly brief) as installable tools for Claude Desktop, Hermes Agent, OpenClaw, Cline, Continue.dev. Handles the 402 → EIP-3009 sign → 200 retry flow transparently; every response carries a signed receipt + on-chain tx hash. ([source](https://github.com/ChrisDover/mako-mcp-server))
- [mako-verifier (Python SDK)](https://github.com/ChrisDover/mako-verifier) - Reference Python verifier for MAKO's signed envelopes (EIP-191 over RFC 8785 canonical JSON). Drop-in receipt verification for any x402 buyer agent or compliance pipeline.

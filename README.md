...
- [x402trace](https://github.com/fardinvahdat/x402trace) - Local CLI debugger for x402 payment flows on Base. Catches the canonical reconciliation bug (wallet debited but server thinks payment failed, [coinbase/x402#1062](https://github.com/coinbase/x402/issues/1062)), pre-flights wallets with `validate`, and explains failed 402s with actionable fixes via `explain`. `npx x402trace`. ([npm](https://www.npmjs.com/package/x402trace))
